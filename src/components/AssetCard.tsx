...
function AssetCard({ asset, isEditing }: AssetCardProps) {
  ...
  return (
    <>
      ...
      <div className="bg-white border border-[#e5e5e5] rounded-xl shadow-xs overflow-hidden flex flex-col lg:flex-row hover:shadow-xs transition-shadow lg:h-[70vh]">
        <div
          className="lg:w-2/5 bg-white flex flex-col border-b lg:border-r border-[#e5e5e5] relative min-h-[200px] max-h-[40vh] lg:max-h-none lg:h-full"
        ...
        <div className="lg:w-3/5 p-3 flex flex-col lg:h-full min-h-0">
          <div className="flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-3">
              <div>
                <span className="block text-[8px] font-bold text-neutral-400 uppercase tracking-widest font-mono">Contributor</span>
                <span className="text-xs font-bold text-neutral-800">{asset.uploadedByName}</span>
              </div>
            </div>
            <div>
              <span className="block text-[8px] font-bold text-neutral-400 uppercase tracking-widest font-mono text-right mb-0.5">Status</span>
              {isDesigner ? (
          ...
          <div className="flex-1 flex flex-col gap-2 min-h-0 py-2">
            <div className="flex-1 flex flex-col min-h-0">
              <label className="block text-[9px] font-bold text-neutral-500 uppercase tracking-wider mb-0.5 font-mono">Caption Copywriter</label>
              <div className="relative flex-1 min-h-0">
                <textarea
                  className="w-full h-full min-h-[60px] text-xs font-mono p-2 border border-[#d4d4d4] rounded-lg focus:outline-none focus:border-neutral-900 resize-none bg-neutral-50"
                  value={localCaption}
            ...
            {asset.status !== AssetStatus.Ready && (
              <>
                <div className="flex-1 flex flex-col min-h-0">
                  <label className="block text-[9px] font-bold text-neutral-500 uppercase tracking-wider mb-0.5 font-mono">Revised Caption</label>
                  <div className="relative flex-1 min-h-0">
                    <textarea
                      className="w-full h-full min-h-[60px] text-xs font-mono p-2 border border-[#d4d4d4] rounded-lg focus:outline-none focus:border-neutral-900 resize-none bg-neutral-50"
                ...
                <div className="flex-1 flex flex-col min-h-0">
                  <label className="block text-[9px] font-bold text-neutral-500 uppercase tracking-wider mb-0.5 font-mono">Comment for Artwork</label>
                  <textarea
                    className="w-full h-full flex-1 min-h-[60px] text-xs font-mono p-2 border border-[#d4d4d4] rounded-lg focus:outline-none focus:border-neutral-900 resize-none bg-neutral-50"
                    value={localComment}
          ...
          <div className="flex-shrink-0">
            <label className="block text-[9px] font-bold text-neutral-500 uppercase tracking-wider mb-0.5 font-mono">Release Date</label>
            <input type="date" className="w-full text-xs p-2 bg-neutral-50 border border-[#d4d4d4] rounded-md focus:outline-none focus:border-neutral-900" value={localDate} onChange={(e) => setLocalDate(e.target.value)} />
          </div>
          ...
          <div className="flex items-center justify-between flex-shrink-0 pt-2 border-t border-neutral-100 mt-2">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-xs text-neutral-500 hover:text-neutral-900 border border-neutral-200 rounded-lg px-2 py-1 hover:bg-neutral-50 transition-colors flex items-center gap-1"
              ...
              <button
                ...
                className="text-xs text-neutral-500 hover:text-neutral-900 border border-neutral-200 rounded-lg px-3 py-1.5 hover:bg-neutral-50 transition-colors flex items-center gap-1"
                title="Download file"
              >
                <Download className="w-3.5 h-3.5" />Download
              </button>
            </div>
            <button type="button" onClick={() => deleteAsset(asset.id)} className="p-1.5 text-neutral-400 hover:text-red-600 rounded" title="Purge Asset"><Trash2 className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
    </>
